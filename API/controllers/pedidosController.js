...
// Middleware para validar entradas al crear un pedido
const validateCreateOrder = [
    body('id_negocio').isInt().withMessage('El ID del negocio debe ser un número entero.'),
    body('productos').isArray().withMessage('Los productos deben ser un array.'),
    body('productos.*.id_producto').isInt().withMessage('El ID del producto debe ser un número entero.'),
    body('productos.*.cantidad').isInt({ gt: 0 }).withMessage('La cantidad debe ser un número entero mayor que 0.'),
...
exports.obtenerPedidosPorNegocio = async (req, res) => {
    ...
    if (!Number.isInteger(Number(id_negocio))) {
